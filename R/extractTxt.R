...
ExtractTxt <- function(articlesHtml, metadata, numberOfTitleCharacters = 80, textToBeRemoved = "", removeEverythingAfter = "", removePunctuationInFilename = TRUE) {
  ...
  txtFilenames <- paste0(file.path(nameOfProject, nameOfWebsite, "Txt", paste0(paste(metadata$dates, metadata$nameOfWebsite, metadata$articlesId, substring(titles, 1, numberOfTitleCharacters), sep = " - "), ".txt")))
