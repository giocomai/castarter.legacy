AnalyseTerms <- function(corpus, nameOfProject, specificTerms, mode = "graph", includeOnly = "", order = "", tipology = "", frequency = "relative") {
  ...
  if (mode == "graph") {
    if (tipology != "") {
      mostFrequentByWebsiteLong <- melt(mostFrequentByWebsite, id.vars=c("nameOfWebsite", "typeOfWebsite"), measure.vars=specificTerms, value.name = "frequency")        
    } else {
    ...
    mostFrequentByWebsite <- ggplot(mostFrequentByWebsiteLong, aes(x=nameOfWebsite, y=frequency, fill=Term)) +
      geom_bar(stat="identity", position="dodge", colour="black") +
      ggtitle(paste("Frequency of", mentionSpecificTerms)) +
      scale_x_discrete (name = "") +
      scale_y_continuous(name = "Frequency of term as % of all words", labels = percent) +
