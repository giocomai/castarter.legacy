...
CountPublicationsPerWebsite <- function(allDatasets, order = "averagePublicationsPerDay", mode = "graph", typeOfWebsite = TRUE, keepOnly = "", removeWebsite = "") {
  ...
  for (i in 1:length(allNamesOfWebsites)) {
    ...
    if (keepOnly[1] != "") {
      numberOfPublicationsPerWebsite$numberOfPublications[i] <- as.integer(plyr::count(allDatasets, "nameOfWebsite")$freq[is.element(allNamesOfWebsitesOriginal, removeWebsite)][i])
    }
    if (removeWebsite[1] != "") {
      numberOfPublicationsPerWebsite$numberOfPublications[i] <- as.integer(plyr::count(allDatasets, "nameOfWebsite")$freq[!is.element(allNamesOfWebsitesOriginal, removeWebsite)][i])
    } else {
  ...
  if (mode == "graph") {
    numberOfPublicationsPerWebsite <- ggplot(numberOfPublicationsPerWebsite, aes(x=nameOfWebsite, y=averagePublicationsPerDay), order = sample(seq_along(averagePublicationsPerDay))) + geom_bar(stat = "identity") + coord_flip()
