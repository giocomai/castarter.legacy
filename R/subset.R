...
# SubsetByDate <- function(corpus) {
#   listOfDates <- seq(start(docSeries), end(docSeries), "DSTday")
#   byDate <- data.frame(matrix(NA, nrow = length(corpus), ncol = length(listOfDates)))
#   names(byDate) <- listOfDates
#   for (i in 1:length(listOfDates)) {
#     byDate[, i] <- meta(corpus, "datetimestamp") == listOfDates[i]
#   }
#   byDate
# }
