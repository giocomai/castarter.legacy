...
CombineWords <- function(corpus, wordCombinations) {
  ...
  #   ## Export word combinations allowing editing in an external editor
  #   if (!file.exists(file.path(nameOfProject, paste(nameOfProject, "wordCombinations.csv")))) {
  #     write.csv(wordCombinations, file = file.path(nameOfProject, paste(nameOfProject, "wordCombinations.csv")), row.names = FALSE)
  #   }
  #   ## Import back word combinations
  #   wordCombinations <- read.csv(file = file.path(nameOfProject, paste(nameOfProject, "wordCombinations.csv")), header = TRUE, stringsAsFactors = FALSE)
...
# AddWordsToBeUnited <- function(wordsToBeCombined, nameOfProject) {
#   if (file.exists(file.path(nameOfProject, paste(nameOfProject, "wordsToBeUnited.txt")))) {
#     wordCombinations <- as.vector(readLines(con = file.path(nameOfProject, paste(nameOfProject, "wordsToBeUnited.txt"))))
#   } else {
#     writeLines(wordsToBeCombined, con = file.path(nameOfProject, paste(nameOfProject, "wordsToBeUnited.txt")))
#   }
#   if (exists("wordsToBeUnited") == FALSE) {
#     wordCombinations <- wordsToBeCombined
#   } else {
#     wordCombinations <- c(wordCombinations, wordsToBeCombined)    
#   }
#   wordCombinations <- unique(as.character(wordCombinations[wordCombinations != ""]))
#   writeLines(wordCombinations, con = file.path(nameOfProject, paste(nameOfProject, "wordsToBeUnited.txt")))
#   wordCombinations
# }
# 
# UniteWords <- function(corpus, wordCombinations) {
#   # Export metadata
#   author <- vector()
#   datetimestamp <- vector()
#   heading <- vector()
#   id <- vector()
#   language <- vector()
#   origin <- vector()
#   for (i in 1:length(corpus)) {
#     author[i] <- meta(corpus[[i]], tag = "author")
#     datetimestamp[i] <- meta(corpus[[i]], tag = "datetimestamp")
#     heading[i] <- meta(corpus[[i]], tag = "heading")
#     id[i] <- meta(corpus[[i]], tag = "id")
#     language[i] <- meta(corpus[[i]], tag = "language")
#     origin[i] <- meta(corpus[[i]], tag = "origin")
#   }
#   wordCombined <- gsub("\\s+","", wordCombinations)
#   for (i in 1:length(wordCombinations)) {
#     word1 <- wordCombinations[i]
#     word2 <- wordCombined[i]
#     for (i in 1:length(corpus)) {
#       corpus[[i]] <- gsub(word1, word2, corpus[[i]], fixed = TRUE)
#     }  
#   }
#   corpus <- VCorpus(VectorSource(corpus))
#   for (i in 1:length(corpus)) {
#     meta(corpus[[i]], tag = "author") <- author[i]  
#     meta(corpus[[i]], tag = "datetimestamp") <- datetimestamp[i]
#     meta(corpus[[i]], tag = "heading") <- heading[i] 
#     meta(corpus[[i]], tag = "id") <- id[i]  
#     meta(corpus[[i]], tag = "language") <- language[i]  
#     meta(corpus[[i]], tag = "origin") <- origin[i]  
#   }
#   corpus
# }
...
CleanCorpus <- function(corpus, stripWhitespace = TRUE, toLowerCase = TRUE, removeNumbers = TRUE, removePunctuation = TRUE, removeControlCharacters = TRUE, removeCharacter = "") {
  if (toLowerCase == TRUE) {
...
EditStemmingDictionary <- function(corpus, nameOfProject, stemmingEditMode = "fromCsv") {
  ...
  stemmingDictionary <- data.frame(row.names=colnames(dtm), "occurrences"=col_sums(dtm), "stemmedTerm"=wordStem(colnames(dtm), language), "stopword"=ifelse(colnames(dtm) %in% stopwords, "stopword", ""), stringsAsFactors = FALSE)
  ...
  if (stemmingEditMode != "none") {
    ...
    for (i in 1:length(stemmingDictionaryMissing)) {
      if (stemmingDictionaryMissing[i] == FALSE)
        stemmingDictionary$stemmedTerm[i] <- as.character(stemmingDictionaryMerged$stemmedTerm.y[i])
      else
        stemmingDictionary$stemmedTerm[i] <- as.character(stemmingDictionaryMerged$stemmedTerm.x[i])
    }
