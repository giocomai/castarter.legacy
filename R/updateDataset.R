...
UpdateDataset <- function(dataset, nameOfProject, nameOfWebsite, numberOfIndexPages = 10, wget = FALSE, wait = 3) {
    ...
    if (gtools::invalid(params$param[params$args=="divClassDatesXpath"]) == TRUE & gtools::invalid(params$param[params$args=="spanClassDatesXpath"]) == TRUE & gtools::invalid(params$param[params$args=="customXpathDates"]) == TRUE) {
        ...
        dates <- ExtractDatesXpath(articlesHtml = articlesHtml[sum(length(dataset$articlesLinks), 1):length(articlesHtml)], dateFormat = params$param[params$args=="dateFormat"], divClass = params$param[params$args=="divClassDatesXpath"], spanClass = params$param[params$args=="spanClassDatesXpath"], customXpath = params$param[params$args=="customXpathDates"], language = dataset$language[1], customString = params$param[params$args=="customStringExtractDates"], minDate = params$param[params$args=="minDate"], maxDate = params$param[params$args=="maxDate"], keepAllString = params$param[params$args=="keepAllString"])
    }
