...
UpdateDataset <- function(dataset, nameOfProject, nameOfWebsite, numberOfIndexPages = 10, wget = FALSE, wait = 3) {
    ...
    indexLinks <- CreateLinks(linkFirstChunk = params$param[params$args=="linkFirstChunk"], linkSecondChunk = params$param[params$args=="linkSecondChunk"], startPage = params$param[params$args=="startPage"], endPage = sum(as.integer(params$param[params$args=="startPage"]), numberOfIndexPages), increaseBy = params$param[params$args=="increaseBy"], sortIndexLinks  = params$param[params$args=="sortIndexLinks"])
