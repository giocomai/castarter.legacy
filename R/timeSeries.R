CreateTimeSeries <- function(allDatasets, corpus, specificTerm, specificWebsites = "", startDate = "", rollingAverage = 30) {
  ...
  termSeries <- zoo(frequencyOfSpecificTerm/c(tapply(row_sums(corpusDtm), time, sum)), 
                    order.by=as.POSIXct(rownames(frequencyOfSpecificTerm)))
  ...
  autoplot(termSeries, facets = NULL) +
    ggtitle(paste("Time series of references to", dQuote(specificTerm))) + scale_x_datetime("Date") + theme(plot.title = element_text(size = rel(1.8)), legend.title = element_text(size = rel(1.5)), legend.text = element_text(size = rel(1))) + scale_colour_brewer(type = "qual", palette = 6)
}