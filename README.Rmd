...
```
N.B. All `castarter` time series function default to a rolling average of 90 days. This can be changed, for example `ShowDistribution(dataset, , rollingAverage = 90)` or `ShowDistribution(dataset, , rollingAverage = 1)`
...
`castarter` graphs are generally created with `ggplot` (and can thus be edited inside R), and can be exported as .png or .svg file for further editing in other applications by adding `export = TRUE` option (graphs are automatically saved in the `outputs` subfolder).
